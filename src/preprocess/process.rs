...
use super::{Macro, PPResult, PreProcessor, TokenStream};
...
impl PreProcessor {
    pub fn process_token<'a>(&mut self, ts: &mut TokenStream<'a>) -> PPResult<Option<Vec<LocationHistory<PreprocessingToken>>>> {
        ...
        match &next.value {
            ...
            PreprocessingToken::Whitespace(_) => {
                output.push(next.same(PreprocessingToken::Whitespace(' ')));
            }
