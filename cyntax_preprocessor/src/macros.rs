use std::{collections::HashMap, fmt::Debug, time::Instant};

use cyntax_common::{
    ast::{Punctuator, Token},
    spanned::Spanned,
};
use cyntax_errors::{Diagnostic, errors::SimpleError};
use cyntax_lexer::{lexer::Lexer, span};

use crate::{
    expand::{ExpandControlFlow, Expander, PResult, ReplacementList},
    prepend::PrependingPeekableIterator,
    tree::TokenTree,
};
impl<'src, I: Debug + Iterator<Item = TokenTree>> Expander<'src, I> {
    pub fn parse_parameters<'func>(&mut self, parameter_token: Spanned<Token>) -> PResult<Vec<String>> {
        if let span!(Token::Delimited { opener, closer, inner_tokens }) = parameter_token {
            let no_whitespace = inner_tokens.iter().filter(|token| !matches!(token, span!(Token::Whitespace(_))));
            let parameters = self.split_delimited(no_whitespace);
            dbg!(&parameters);
            for parameter in &parameters {
                if parameter.value.len() == 0 {
                    return Err(SimpleError(parameter.range.clone(), format!("empty parameter?")).into_why_report());
                }
                if parameter.value.len() != 1 {
                    // Err(SimpleError(parameter))
                    panic!("parameter must be exactly one identifier");
                }
            }
            let parameters_as_strings = parameters
                .into_iter()
                .map(|argument| match argument.value.first().unwrap() {
                    span!(Token::Identifier(identifier)) => identifier,
                    // above is a check to make sure that each parameter is exactly one token, and its just an identifier
                    _ => unreachable!(),
                })
                .cloned()
                .collect();
            Ok(parameters_as_strings)
        } else {
            Err(SimpleError(parameter_token.range, "internal compiler error: tried to parse parameters but its not a delimited".to_string()).into_why_report())
        }
    }
    /// Splits a comma-delimited sequence of tokens into groups.
    ///
    /// - `[]` -> `[]`
    /// - `[,,]` -> `[[], [], []]`
    /// - `[2+5]` -> `[[2+5]]`
    /// - `[2+5,]` -> `[[2+5], []]`
    pub fn split_delimited<'a, L: Iterator<Item = &'a Spanned<Token>>>(&self, mut tokens: L) -> Vec<Spanned<Vec<&'a Spanned<Token>>>> {
        let mut elements = vec![];
        let mut current_element = vec![];

        let mut s = 0;
        while let Some(token) = tokens.next() {
            match token {
                span!(comma, Token::Punctuator(Punctuator::Comma)) => {
                    elements.push(Spanned::new(s..comma.end, std::mem::replace(&mut current_element, Vec::new())));
                }
                _ => {
                    s = token.range.start;
                    current_element.push(token);
                }
            }
        }
        elements.push(Spanned::new(0..0, std::mem::replace(&mut current_element, Vec::new())));

        // manual override for `[]` -> `[]`
        // this can probably be fixed algorithmically
        if elements.len() == 1 && elements.first().unwrap().value.len() == 0 { vec![] } else { elements }
    }
}
