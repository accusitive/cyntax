...
use cyntax_hir::{self as hir, Expression, HirId, Ty};

use crate::{visit::Visitor, AstLower};

pub struct TyCheckVisitor<'src, 'ctx, 'hir> {
    map: HashMap<HirId, &'hir Ty<'hir>>,
    lowering: &'ctx AstLower<'src, 'hir>,
...
impl<'src, 'ctx, 'hir> TyCheckVisitor<'src, 'ctx, 'hir> {
    pub fn new(ctx: &'ctx AstLower<'src, 'hir>) -> Self {
        Self{
            map: HashMap::new(),
            lowering: ctx,
            diagnostics: vec![]
    ...
    fn visit_expression(&mut self, expr: &'hir cyntax_hir::Expression<'hir>) {
        match &expr.kind {
            ...
            cyntax_hir::ExpressionKind::BinaryOp(op, left, right) => {
                ...
                self.diagnostics.push(MismatchedTypeErr{location: expr.loc.clone(), left_loc: &left.loc, right_loc: &right.loc, left: lty, right: rty}.into_codespan_report());
                // panic!();
            },
...
impl<'b, 'hir> Diagnostic for MismatchedTypeErr<'b, 'hir> {
    ...
    fn labels(&self) -> Vec<cyntax_errors::Label> {
        ...
        labels.push(Label { kind: cyntax_errors::LabelKind::Secondary, location: self.left_loc.clone(), message: format!("left has type {}", self.left) });
        labels.push(Label { kind: cyntax_errors::LabelKind::Secondary, location: self.right_loc.clone(), message: format!("right has type {}", self.right) });
